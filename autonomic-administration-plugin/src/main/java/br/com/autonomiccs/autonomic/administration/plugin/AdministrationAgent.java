 ...
 * the administration task upon the cluster. Be aware that part of those tasks are controlled by the
 * heuristics loaded into the agent.
 */
...
public class AdministrationAgent implements InitializingBean {
    ...
    /**
     * Time in seconds to wait for migrations. As the agent will not be doing nothing in this
     * cluster for awhile, it sleeps {@value} seconds.
     */
    private static final int TIME_TO_WAIT_FOR_MIGRATIONS = 10;
     ...
     * {@link ClusterAdministrationHeuristicAlgorithm} to be used and calls the
     ...
     */
    public void workOnCluster(ClusterVO cluster, ClusterAdministrationHeuristicAlgorithm administrationAlgorithm) {
    ...
    protected void mapAndExecuteVMsMigrations(ClusterVO cluster, ClusterAdministrationHeuristicAlgorithm administrationAlgorithm) {
        ...
        logger.info(String.format("Waiting few [%d] seconds after VMs migration for cluster[id=%d].", TIME_TO_WAIT_FOR_MIGRATIONS, clusterId));
        threadUtils.sleepThread(TIME_TO_WAIT_FOR_MIGRATIONS);//TODO review
    }
