...
public class AdministrationAgentTest {
    ...
    @Test
    public void receiveClusterToBeManagedTestClusterIdNotNullButClusterIsNull() {
    ...
    public void getClusterToManageTestEmptyEnabledZonesList() {
        ...
        Assert.assertNull(result);
        verifyGetClusterToManageCalledMethods(0);
    }
    ...
    public void getClusterToManageTestNotEmptyEnabledZonesList() {
        ...
        Assert.assertNotNull(result);
        verifyGetClusterToManageCalledMethods(1);
    }
    ...
    public void getClusterToManageFromZoneTestListOfClustersEmpty() {
        ...
        administrationAgent.getClusterToManageFromZone(clusterAdministrationHeuristicAlgorithm, null);
    ...
    @Test
    public void workOnClusterTest() {
        ClusterVO cluster = createCluster(null);
        administrationAgent.workOnCluster(cluster, clusterAdministrationHeuristicAlgorithm);
    ...
    public void shutdownHostTestHostWithoutVms() {
        ...
        verifyShutdownHostCalledMethods(1);
    }
    ...
    public void shutdownHostTestHostWithVms() {
        ...
        vms.add(vm);
        setupShutdownHostTest(vms);

        administrationAgent.shutdownHost(0l);

        verifyShutdownHostCalledMethods(0);
    }
    ...
    private void verifyGetClusterToManageCalledMethods(int times) {
        Mockito.verify(zoneService).listAllZonesEnabled();
        Mockito.verify(administrationAgent, Mockito.times(times)).getClusterToManageFromZone(Mockito.any(ClusterAdministrationHeuristicAlgorithm.class), Mockito.anyLong());
    }
    ...
    private void shutdownIdleHostsTestSetupMethodsExecution(List<HostResources> idleHosts, boolean canHeuristicShutdownHosts,
            boolean canPowerOffAnotherHostInCloud, boolean canPowerOffHost) {
    ...
    private void verifyMapAndExecuteVMsMigrationsCalledMethods(int rankHostsTimes, int findHostByIdTimes) {
        ...
        Mockito.verify(threadUtils, Mockito.times(rankHostsTimes)).sleepThread(Mockito.anyInt());
    }
    ...
    private void verifyShutdownHostCalledMethods(int timesIfHostEmpty) {
        Mockito.verify(administrationAgent).checkIfHostIsUpAndEnabled(Mockito.anyLong());
        Mockito.verify(hostService).listAllVmsFromHost(Mockito.anyLong());
        Mockito.verify(administrationAgent, Mockito.times(timesIfHostEmpty)).putHostInMaintenance(Mockito.anyLong());
        Mockito.verify(hostService, Mockito.times(timesIfHostEmpty)).findHostById(Mockito.anyLong());
        Mockito.verify(hypervisorManager, Mockito.times(timesIfHostEmpty)).shutdownHost(Mockito.any(HostVO.class));
    }
